/****************************************************************/
/*    NAME: rerun_MONICA_combined_belfast.sas                   */
...
data work.datatorun;
   ...
   if centre=34 then output work.datatorun; * select Belfast data; 
run;
data work.datatorun;
   set work.datatorun;
...
* Frequency at 2.8, just above limit of periodogram, but not below the spectrum;

*** PLOTS ***;
* a) Trend and CI;
filename graph1 "U:\Research\Projects\ihbi\aushsi\aushsi_barnetta\meta.research\reproducibility.challenge\updated_files\plots\trend_combined_belfast.eps";
...
axis2 minor=NONE label=(a=90 'Data') order=(30 to 100 by 10);
...
data work.posterior;
   set work.posterior;
   year = 83+ (time-1)/12; * add back year;
run;
filename graph1 "U:\Research\Projects\ihbi\aushsi\aushsi_barnetta\meta.research\reproducibility.challenge\updated_files\plots\estimates_combined_belfast.eps";
goptions reset=all ftext=centx htext=3 gunit=pct colors=(black) gunit=pct noborder
    gsfname=graph1 noprompt gsfmode=replace device=PSLEPSF fontres=presentation dash;
axis1 minor=NONE label=('Year');
axis2 minor=NONE label=(a=90 'CHD event rate per 100,000') order=(30 to 100 by 10);
