...
* One centre;
data work.datatorun;
   set work.sumevent(rename=(ratestd=var)); * select Perth data; *rename for macro;
   if centre=10 then output work.datatorun;
run;
data work.datatorun;
   set work.datatorun;
...
* Add a cycle of 2.3 based on the periodogram test (large spike);
options nonotes nomprint nosymbolgen; *<-reduce output to log;
%runfil(k=2, freq1=12, freq2=2.3, freq3=-99, freq4=-99, freq5=-99, MCMC=5000, burn=500);
options notes mprint symbolgen; *<-restore output to log;

%boottest; * Test the residuals for remaining seasonal structure;

* Add a cycle of 5.5 based on the periodogram test (also spike in spectrum);
options nonotes nomprint nosymbolgen; *<-reduce output to log;
%runfil(k=3, freq1=12, freq2=2.3, freq3=5.5, freq4=-99, freq5=-99, MCMC=5000, burn=500);
options notes mprint symbolgen; *<-restore output to log;

%boottest; * Test the residuals for remaining seasonal structure;

* Add a cycle of 3.8 based on the periodogram test (also spike in spectrum);
options nonotes nomprint nosymbolgen; *<-reduce output to log;
%runfil(k=4, freq1=12, freq2=2.3, freq3=5.5, freq4=3.8, freq5=-99, MCMC=5000, burn=500);
options notes mprint symbolgen; *<-restore output to log;

%boottest; * Test the residuals for remaining seasonal structure;
* Spikes at 2.7 and 4.6, but only just over periodogram threshold;

*** PLOTS ***;
* a) Trend and CI;
filename graph1 "U:\Research\Projects\ihbi\aushsi\aushsi_barnetta\meta.research\reproducibility.challenge\updated_files\plots\trend_combined_perth.eps";
...
axis2 minor=NONE label=(a=90 'Data') order=(10 to 50 by 10);
...
run;
filename graph1 "U:\Research\Projects\ihbi\aushsi\aushsi_barnetta\meta.research\reproducibility.challenge\updated_files\plots\estimates_combined_perth.eps";
goptions reset=all ftext=centx htext=3 gunit=pct colors=(black) gunit=pct noborder
    gsfname=graph1 noprompt gsfmode=replace device=PSLEPSF fontres=presentation dash;
axis1 minor=NONE label=('Year');
axis2 minor=NONE label=(a=90 'CHD event rate per 100,000') order=(10 to 50 by 10);
